...
import { CheckCircle2, ChevronLeft, ChevronRight, Lock, AlertCircle } from "lucide-react";
...
export const Route = createFileRoute("/vote")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Cast your vote in the school election." },
    ],
  }),
  component: VotePage,
});
...
function VotePage() {
  ...
  if (state.isLoading) return <FullScreenMessage title="Connecting to Election Authority..." />;
  if (state.status === "draft") return <FullScreenMessage title="Election has not started yet." />;
...
function VotingKiosk({ deviceId }: { deviceId: string }) {
  ...
  function toggleSelection(candidateId: string) {
    ...
    setSelections((prev) => {
      ...
      if (list.includes(candidateId)) {
        return { ...prev, [currentPos.id]: list.filter((id) => id !== candidateId) };
      }
  ...
  async function submitVotes() {
    ...
    for (const [positionId, candIds] of Object.entries(selections)) {
      for (const candidateId of candIds) picks.push({ positionId, candidateId });
    }
      ...
      <header className="border-b border-white/10 px-6 py-4">
        <div className="mx-auto flex max-w-5xl items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-extrabold uppercase tracking-tighter">Gentanjali school voting</span>
          </div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div className="max-w-md rounded border border-white/10 bg-ink p-8 text-center">
            <h2 className="text-2xl font-extrabold tracking-tight">Confirm submission</h2>
...
function CodeScreen({
  ...
  return (
    <div className="animate-ballot mx-auto max-w-2xl text-center">
      ...
      <h1 className="mt-3 text-5xl font-extrabold tracking-tighter">Cast Your Ballot</h1>
    ...
    <div className="animate-ballot">
      <div className="mb-6 flex items-end justify-between">
        <div>
          ...
          <h2 className="mt-2 text-4xl font-extrabold tracking-tight">{position.name}</h2>
        </div>
        ...
        {Array.from({ length: totalPositions }).map((_, i) => (
          <div
            ...
            className={`h-1 flex-1 rounded-full ${
              i < positionIndex ? "bg-primary" : i === positionIndex ? "bg-white" : "bg-white/10"
            }`}
          />
        ))}
      </div>
...
function ReviewScreen({
  ...
  return (
    <div className="animate-ballot">
      ...
      <h2 className="mt-2 text-4xl font-extrabold tracking-tight">Review your ballot</h2>
      <p className="mt-2 text-sm text-white/60">
        Confirm your selections below. After submission your vote cannot be changed.
      </p>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          {c.photo && (
                            <img src={c.photo} alt={c.name} className="size-8 rounded-full object-cover border border-white/10" />
                          )}
                          <span className="font-bold">{c.name}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          {c.symbol && (
                            <img src={c.symbol} alt={c.symbolName} className="size-4 object-contain" />
                          )}
                          <span className="font-mono text-[10px] uppercase tracking-widest text-white/40">
                            {c.symbolName}
                          </span>
...
function ThanksScreen() {
  return (
    <div className="animate-ballot py-24 text-center">
      ...
      <h2 className="mt-8 text-5xl font-extrabold tracking-tighter">Thank you for voting</h2>
      <p className="mt-4 text-base text-white/60">Your vote has been submitted successfully.</p>
